...
from .models import AuditEvent, EmailVerificationToken, PasswordResetToken, RecoveryCode, Session, User
from .password_reset import generate_reset_token, hash_reset_token
from .schemas import (
...
async def register(payload: RegisterRequest, db: AsyncSession = Depends(get_db)) -> User:
    ...
    db.add(user)
    await db.flush()
    await _create_email_verification(user, db)
...
@app.post("/v1/auth/forgot-password", response_model=MessageResponse)
...
async def reset_password(payload: ResetPasswordRequest, db: AsyncSession = Depends(get_db)) -> MessageResponse:
    ...
    db.add(AuditEvent(actor_id=user.id, action="identity.password_reset.completed", resource_type="user", resource_id=str(user.id)))
    await db.commit()
