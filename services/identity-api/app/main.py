...
from .schemas import EmailVerificationRequest, LoginRequest, MessageResponse, RefreshRequest, ResendVerificationRequest, TokenResponse, UserResponse
...
async def logout(user: User = Depends(current_user), credentials: HTTPAuthorizationCredentials | None = Depends(bearer), db: AsyncSession = Depends(get_db)) -> MessageResponse:
    ...
    session_id = UUID(decode_token(credentials.credentials)["sid"])
